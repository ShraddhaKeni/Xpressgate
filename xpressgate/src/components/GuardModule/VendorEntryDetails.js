...
const VendorEntryDetails = () => {
    ...
    const getData =async()=>{
        ...
        try {
            const {data} = await axios.get(`${process.env.REACT_APP_SERVER_PATH}/api/vendorlist/getOne/${location.state.id}`)
        ...
        try {
          bookings.map(async(items)=>{
            try {
              ...
              const {data} = await axios.post(`${process.env.REACT_APP_SERVER_PATH}/api/inout/add`,submitData)
              console.log(data)
              const bookingUpdate = await axios.get(`${process.env.REACT_APP_SERVER_PATH}/api/bookvendor/removeBooking/${items.booking_id}`)
